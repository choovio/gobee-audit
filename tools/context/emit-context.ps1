...
$rules = @()
foreach($pat in $ruleFiles){
  $rules += Get-ChildItem -Path $pat -ErrorAction SilentlyContinue | Select-Object -Expand FullName
}
...
foreach($root in $RepoRoots){
  foreach($t in $targets){
    $p = Join-Path $root $t
    if(Test-Path $p){
      $hits += [pscustomobject]@{ repo=(Split-Path $root -Leaf); path=$p; file=(Split-Path $p -Leaf) }
    }
  }
}
